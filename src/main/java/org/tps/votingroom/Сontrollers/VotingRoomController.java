package org.tps.votingroom.Сontrollers;

import com.fasterxml.jackson.core.JsonProcessingException;
import com.fasterxml.jackson.databind.ObjectMapper;
import org.tps.votingroom.Exceptions.AlreadyExistingException;
import org.tps.votingroom.DataBase.LineInformationDataBase;
  ...
  private void getInformationAboutVotingRoom() {
    service.get(
        ...
        (Request request, Response response) -> {
          ...
          try {
            findInformationRoomResponse = dataBaseService.getLinePostgres(votingRoomId);
  ...
  private void vote() {
    service.post(
        ...
        (Request request, Response response) -> {
          ...
          try {
            informationAboutRoom = dataBaseService.getLinePostgres(votingRoomId);
          } catch (SQLException e) {
            response.status(404);
            LOG.error("Cannot find room with id={}", votingRoomId);
            return objectMapper.writeValueAsString(e.getMessage());
          ...
          if (informationAboutRoom.state() == 0) {
            ...
            Integer previousValue = informationAboutRoom.variantsInterestRate().get(position - 1);
  ...
  private void appendFriendToRoom() {
    service.post(
        ...
        (Request request, Response response) -> {
          ...
          LineInformationDataBase lineInformationDataBase = dataBaseService.getLinePostgres(votingRoomId);
  ...
  private void endVoting() {
    service.put(
        ...
        (Request request, Response response) -> {
          ...
          if (!userId.equals(findInformationRoomResponse.ownerId())) {
            response.status(403);
